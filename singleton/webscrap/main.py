...
class Singleton(object):
    ...
    Args:
        object (_type_): object for creating singleton
    """
    def __new__(cls):
...
class ImageDownloaderThread(threading.Thread):
    '''Next, we’ll create a class for creating a thread. In this thread,
     we’ll download images from the website:'''
...
def traverse_site(max_links=10):
    '''traverses the website using BFS algorithms, finds links, and adds them to a set for further downloading. 
...
def download_images(thread_name):
    '''downloads images from the last web resource page in the singleton.to_visit queue and 
    saves it to the img directory. Here, we use a singleton for synchronizing shared data, which is a set of pages to visit between two threads:'''
    singleton = Singleton()
